...
class Md_Site_Stats_Widget_Wp_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// Enqueue the jQuery script for Ajax.
		...
		// Localize script: passing parameters to Ajax script to build the request.
		wp_localize_script(
			'stats_widget',
			'wpApiSettings',
			array(
		...
		// Every ite statistic are grouped in blocks visible with accordio effect. Here is the tag where to create the block/s.
	...
	public function form( $instance ) {
		...
		$id_refresh = $this->get_field_id( 'refresh' );
		$name_refresh = $this->get_field_name( 'refresh' ); ?>
		<p>
		<label for="<?php echo absint( $id_title ); ?>"><?php esc_html_e( 'Title:', 'md_site_stats_widget' ); ?></label> 
		<input class="widefat" id="<?php echo absint( $id_title ); ?>" name="<?php esc_html_e( $name_title ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">

		<label for="<?php echo absint( $id_refresh ); ?>"><?php esc_html_e( 'Refresh time:', 'md_site_stats_widget' ); ?></label> 
		<select class="widefat" id="<?php echo absint( $id_refresh ); ?>" name="<?php esc_html_e( $name_refresh ); ?>">
